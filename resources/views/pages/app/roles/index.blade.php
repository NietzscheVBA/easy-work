<x-layouts.app title="Papéis — Controle de Papéis">

    {{-- Header --}}
    <div class="flex items-center justify-between mb-8">
        <x-ui.page-heading title="Papéis" description="Gerencie todos os seus papéis" />
        ...
        <a href="{{ route('admin.role.create') }}"
           class="inline-flex items-center gap-2 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-xl shadow-sm shadow-blue-200 transition-all duration-150 active:scale-95">
            <x-ui.icon name="roles" class="w-4 h-4" />
            Novo Papel
        </a>
    </div>
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden"
     x-data="roleTable(@js($roles->map(fn($u) => [
         'id'           => $u->id,
         'name'         => $u->name,
         'description'  => $u->description,
         'href'   => route('admin.role.show', $u),
     ])->values()))">

    {{-- Search bar --}}
    <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between gap-4">
        <div class="relative flex-1">
            <x-ui.icon name="roles" class="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2" />
            <input
                type="text"
                x-model="search"
                @input="currentPage = 1"
    ...
    <div class="divide-y divide-slate-50 min-h-[3rem]">
        <template x-for="role in paginated" :key="role.id">
            <div class="flex items-center justify-between px-6 py-3 hover:bg-slate-50 transition-colors group">
                <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center text-sm font-bold flex-shrink-0"
                         x-text="role.name.charAt(0).toUpperCase()">
                    </div>
                    <div>
                        <p class="text-sm font-semibold text-slate-700 group-hover:text-blue-600 transition-colors"
                           x-text="role.name"></p>
                        <p class="text-xs text-slate-400" x-text="role.description"></p>
                    </div>
                </div>
    ...
    function roleTable(allRole) {
        return {
            ...
            get filtered() {
                ...
                if (!q) return this.allRole
                return this.allRole.filter(u =>
